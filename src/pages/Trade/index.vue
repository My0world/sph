<template>
  <div class="trade-container">
    ...
    <div class="sub clearFix">
      <a class="subBtn" @click="sumbitOrder(orderInfo, userDefaultAddress)"
        >提交订单</a
      >
    </div>
  </div>
...
export default {
  ...
  computed: {
    ...
    ...mapGetters({ detailArrayList: "trade/detailArrayList" }),
    ...mapState("trade", ["orderInfo"]),
    //将来提交订单最终选中地址
    userDefaultAddress() {
      //查找数组符合元素的并返回
      let result = this.addressInfo.find((item) => {
  ...
  methods: {
    ...
    //提交订单
    async sumbitOrder(orderInfo, userDefaultAddress) {
      //交易编码
      let tradeNo = orderInfo.tradeNo;
      let data = {
        consignee: userDefaultAddress.consignee, //收件人名字
        consigneeTel: userDefaultAddress.phoneNum, //收件人电话号码
        deliveryAddress: userDefaultAddress.fullAddress, //收件人地址
        paymentWay: "ONLINE", //支付方式
        orderComment: this.msg, //留言
        orderDetailList: orderInfo.detailArrayList, //购买的商品清单
      };
